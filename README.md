...
此项仅适用于百度公司内部，用于安装线上Node基础环境，外部用户可以忽略，直接使用[默认启动](#%E9%BB%98%E8%AE%A4%E5%90%AF%E5%8A%A8)
...
Yog2 App的目录规范可以结合 [创建一个Yog2 App](https://github.com/fex-team/yog2#%E5%88%9B%E5%BB%BA%E4%B8%80%E4%B8%AAyog2-app) 创建的目录进行了解
...
插件的配置均存放在Yog项目的 `conf/plugins` 文件夹中，与插件编写规则一致，配置也需要通过属性名显示声明配置所属的插件
...
此外，Yog2还支持根据环境变量加载不同的配置，举例来说

```
export YOG_ENV=dev
```

环境变量 `YOG_ENV` 的值将会影响配置的加载，比如当值为dev时，就会优先加载 `blahblah.dev.js` 而非 `blahblah.js`。我们可以通过这个功能更方便的管理开发环境的配置与测试环境的配置。

除此外还有一个特殊的配置后缀 `default`，当 `blahblah.js` 不存在时，会尝试加载 `blahblah.default.js`。可以在自定义插件时使用，这样用户在 `blahblah.js` 中修改配置后，即使重新安装插件，也不会覆盖用户修改后的配置。
...
在 `YOG2` 下使用 `RAL` 无需处理初始化工作，只需要将配置放置在 YOG2 Project 目录下的 `/conf/ral` 中即可。
